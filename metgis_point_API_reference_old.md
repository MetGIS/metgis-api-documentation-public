...
**Important note: from August 2026, a new version of the Point API is available. This old version is expected to be shut down soon, according to agreement with our existing users.** For a list of changes in each forecast package, see the [transition guide from the old Point API to the new one](data_metgis_point_API_reference/metgis_point_API_change_list.md).
